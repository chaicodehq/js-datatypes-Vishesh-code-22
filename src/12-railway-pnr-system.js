...
export function processRailwayPNR(pnrData) {
    ...
    if (pnrData?.train === undefined) return null;
    ...
    const passengers = pnrData.passengers.map((passenger) => {
        let statusLabel;
        if (
            passenger.current.startsWith("B") ||
            passenger.current.startsWith("S")
        ) {
            statusLabel = "CONFIRMED";
            confirmed++;
        } else if (passenger.current.startsWith("WL")) {
            statusLabel = "WAITING";
            waiting++;
        } else if (passenger.current.startsWith("CAN")) {
            statusLabel = "CANCELLED";
            cancelled++;
        } else if (passenger.current.startsWith("RAC")) {
            statusLabel = "RAC";
            rac++;
        }
        return {
    ...
    const summary = {
        ...
        cancelled,
        rac,
        allConfirmed: confirmed === passengers.length,
        anyWaiting: waiting > 0,
    };
    const chartPrepared = waiting === 0 && rac === 0;
    return {
        pnrFormatted,
        trainInfo,
        passengers,
