...
export function generateLocalPass(passenger) {
    ...
    let keys = ["name", "from", "to", "classType"];

    let hasAllKeys = keys.every((key) => key in passenger);

    if (Object.values(passenger).includes("")) {
        return "INVALID PASS";
    }
    ...
    let from = `${passenger.from[0].toUpperCase()}${passenger.from.slice(1).toLowerCase()}`;
    let to = `${passenger.to[0].toUpperCase()}${passenger.to.slice(1).toLowerCase()}`;
